...
# ---------- LOOKUP DE IMAGEM ----------
# 1) Tenta Windows Server 2022 *Standard*; se não achar, cai para "Server 2022" genérico.
...
resource "oci_core_instance" "win" {
  # AD resolvido automaticamente em ad_auto.tf (local.ad_final)
  ...
  # Injeta user_data apenas se habilitado
  metadata = var.inject_user_data ? {
    user_data = base64encode(
      templatefile(local.userdata_template_abs, {
  ...
  lifecycle {
    ...
    # Falha clara quando o template não existe e inject_user_data=true
    precondition {
